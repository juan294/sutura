...
test('public install uses only sutura@0.3.3 and verifies the independently resolved release SHA', async () => {
  ...
  try {
    ...
    const result = await verifyInstall({
      mode: 'public', root: temporary, releaseVersion: '0.3.3',
    ...
    assert.deepEqual(calls[0], ['resolve', '0.3.3']);
    assert.deepEqual(calls[1], ['pack', 'sutura@0.3.3']);
...
test('public wrapper accepts one explicit exact semver and rejects mutable or ranged releases', async () => {
  assert.equal(parseReleaseVersion([]), '0.3.3');
