...
test('v0.3.2 controller state is ignored before the lock-protected gate runs', async () => {
  const ignore = await readFile('.gitignore', 'utf8');
  assert.match(ignore, /^\.sutura\/placebo-v0\.3\.2-live-ledger\.json$/mu);
  assert.match(ignore, /^\.sutura\/placebo-v0\.3\.2-live\.lock$/mu);
  assert.match(ignore, /^\.sutura\/placebo-v0\.3\.2-live\.lock\.recovery\/$/mu);
  assert.match(ignore, /^\.sutura\/placebo-v0\.3\.2-live-artifacts\/$/mu);
