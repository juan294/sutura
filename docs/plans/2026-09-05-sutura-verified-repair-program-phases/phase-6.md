...
- The standalone NeMo `EvaluationHarness` invocation, `packages/evaluation/scripts/evaluate-atif.py` and its installed smoke test are absent, and this item is **blocked on a toolchain decision that is not mine to make**.

  The inspection the plan asks for is done. At the pinned revision `23cd127dfba56994cd272f2771350d0ec13f3dd1`, `packages/nvidia_nat_eval` exists alongside the already-pinned `nvidia_nat_atif`, and `src/nat/plugins/eval/runtime/eval_harness.py` defines `class EvaluationHarness` with `async def evaluate(self, evaluators: dict[str, AtifEvaluator], ...)`. So the API the pseudocode describes is really there and no replacement revision is needed.

  What blocks it: `packages/evaluation/pyproject.toml` sets `required-version = "==0.12.7"` and the installed toolchain is uv 0.12.9, so `pnpm run test:atif` already fails here with `Required uv version ==0.12.7 does not match the running version 0.12.9`. This is pre-existing and unrelated to this program — `test:atif` is not part of `ci:local`, which is why the gates stay green. Until it is resolved there is no way to run an installed smoke test, and the acceptance item explicitly rejects a hand-written scorer that does not invoke NVIDIA's harness, so writing one would not satisfy it either.

  Resolving it means either pinning uv back to 0.12.7 in this environment or moving the `required-version` pin, both of which change reproducibility for every future run. That is Juan's call, not an incidental fix.
