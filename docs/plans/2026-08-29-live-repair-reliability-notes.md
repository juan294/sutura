...
- Gate stability: the CLI unit test now injects the benchmark result and checks adapter and option forwarding without duplicating the complete 55-result corpus. The harness retains the canonical full-corpus integration, and the corrected 70-test Placebo suite passed twice without the duplicate timeout path.
- Exact package verification: passed from a clean commit and bound the installed `sutura@0.2.0` package and Action to the same exact branch tip.
- Pending: read-only live provider canary, exact-SHA CI, and final dogfood proof. No new dogfood run is authorized before the canary passes.
