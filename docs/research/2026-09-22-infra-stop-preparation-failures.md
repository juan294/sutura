...
| Wrong failed step chosen: an aggregate "all checks passed" gate reproduces green | coach 4 | 7 | **Sutura** | Fixed 2026-09-23, [#151](https://github.com/juan294/sutura/issues/151) |
...
| --- | --- | --- | --- | --- | ---: | --- | ---: | --- |
| coach | 34756213847 | 34755968351 | 09-13 | 26.8 s / 0.2746 | 7 | the aggregate gate job's first `echo` line | 0 | reproduction passed |
| coach | 34756272771 | 34755991410 | 09-13 | 29.8 s / 0.3046 | 7 | same shape | 0 | reproduction passed |
...
| coach | 34856566228 | 34855892464 | 09-14 | 1.28 s / 0.0136 | 4 | pnpm install (network-enabled preparation) | 254 | `ENOENT` opening a `patches/*.patch` file |
...
coach's `package.json` declares a `pnpm.patchedDependencies` entry pointing at
a file under `patches/`. The dependency snapshot
...
### 2. The aggregate gate step is chosen as the failing command — ours, fixed

coach CI run 34755968351 has three failed jobs: a validation job, a test
shard, and an aggregate gate job that runs last. The gate's script starts with
an `echo` of the other jobs' results and exits non-zero because they failed.
Command selection
...
it was filed as [#151](https://github.com/juan294/sutura/issues/151), and fixed on 2026-09-23: failed steps are ranked by completion time, the earliest with its own command is diagnosed, and a green reproduction gets one retry on the next step with a different command.
...
### 4. Git baseline refuses a tracked-but-ignored path — ours, fixed
